...
This function updates the database values in the table counter_stark_gegen depending on the users vote.
...
if ($_POST["tabelle"]=="counter_stark_gegen") {
...
$stmt = $mysqli->prepare("DELETE FROM dg437tzU_Counter_Vote WHERE Time<= ?");
$stmt->bind_param("i", $nichtmehrgueltig);
$stmt->execute();
//Schauen ob IP schon abgestimmt hat
$stmt=$mysqli->prepare("SELECT IP FROM dg437tzU_Counter_Vote WHERE IP=?");
...
if ($db_ip[IP]=="") {
    $stmt = $mysqli->prepare("INSERT INTO dg437tzU_Counter_Vote (Champion,IP,$updatetabelle,Time) VALUES(?,?,?,?) ");
    $counter=$counter_id.",";
...
//Falls neuer Champion, Datenbank updaten;
    ...
    $stmt=$mysqli->prepare("SELECT Champion FROM dg437tzU_Counter_Vote WHERE Champion=? AND IP=? ");
    ...
    if ($db_champ[Champion]=="") {
        ...
        $stmt=$mysqli->prepare("SELECT $updatetabelle FROM dg437tzU_Counter_Vote WHERE Champion=? AND IP=? ");
        $stmt->bind_param(ss, $champname, $ip);
        $stmt->execute();
        $res=$stmt->get_result();
        ...
        if ($vote==1) {
            ...
            $stmt=$mysqli->prepare("UPDATE dg437tzU_Counter_Vote SET $updatetabelle=CONCAT($updatetabelle,$counter_id,\",\") WHERE Champion=? AND IP=? ");
            $stmt->bind_param(ss, $champname, $ip);
            $stmt->execute();
            $stmt->close();
            $stmt = $mysqli->prepare("SELECT $champname FROM $updatetabelle WHERE ChampionID=?");
